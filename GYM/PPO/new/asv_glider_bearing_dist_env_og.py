...
class AsvGliderBearingEnv(gym.Env):
    ...
    interpret as coming from a state estimator in the real system.
    """
    ...
    def __init__(self):
        ...
        max_dist = np.sqrt(2) * self.world_size  # something safely large
        obs_max = np.array(
    ...
    def _get_obs(self):
        ...
        # include ASV GPS position AND distance
        obs = np.array(
            [
                bearing,
                dist,           # <--- NEW distance term
                current[0],
                current[1],
                self.asv_pos[0],
                self.asv_pos[1],
    ...
    def step(self, action):
        ...
        reward = (
            -dist
            - 0.1 * np.linalg.norm(action)
            - 0.2 * np.linalg.norm(current)
        )
