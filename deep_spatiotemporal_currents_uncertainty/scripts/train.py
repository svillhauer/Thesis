...
def uncertainty_loss(pred_mean, pred_logvar, target):
    ...
    Loss = 0.5 * (exp(-s) * (y - mu)^2 + s)
    where s = log(sigma^2)
    """
    # Clamp logvar for numerical stability if needed, though usually safe
...
def evaluate(model, loader, device):
    ...
    for X, Y in loader:
        ...
        # Split channels: 0-1 are Mean (Velocity), 2-3 are LogVar (Uncertainty)
        pred_mean = output[:, :2]
        ...
        # Compute RMSE on the Mean predictions only
        total_loss += float(loss.item()) * X.shape[0]
        total_rmse += float(rmse(pred_mean, Y).item()) * X.shape[0]
        n += X.shape[0]
...
def main():
    ...
    # MODIFICATION: out_ch=4 (u, v, sigma_u, sigma_v)
    model = UNetConvLSTM(in_ch=2, out_ch=4, base=args.base_ch, lstm_ch=args.lstm_ch).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for X, Y in tqdm(train_loader, desc=f"Epoch {epoch}/{args.epochs}", leave=False):
            ...
            # Forward pass
            output = model(X) # (B, 4, H, W)
            
            # Split output
            pred_mean = output[:, :2]   # Channels 0,1 -> u, v
            pred_logvar = output[:, 2:] # Channels 2,3 -> log(sigma^2)
        ...
        train_loss /= max(n,1)
        # Validate (returns NLL as val_loss, RMSE as val_rmse)
        ...
        print(f"[{epoch:03d}] train_nll={train_loss:.6f}  val_nll={val_loss:.6f}  val_rmse={val_rmse:.6f}")

    print("Done. Best val NLL:", best_val)
