...
def parse_args():
    ...
    p.add_argument("--t-index", type=int, required=True, help="Target time index t to predict (uses t-L...t-1)")
...
def main():
    ...
    model = UNetConvLSTM(in_ch=2, out_ch=2, base=int(model_args.get("base_ch", 32)), lstm_ch=int(model_args.get("lstm_ch", 256)))
    model.load_state_dict(ckpt["model"])
    model.to(device).eval()

    with torch.no_grad():
        pred = model(torch.from_numpy(X[None]).to(device)).cpu().numpy()[0]  # (2,H,W)

    # invert normalization for saving
    pred_uv = invert_minmax(pred, s)
    gt_uv   = invert_minmax(Y, s)

    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    np.savez_compressed(
        args.out,
        pred_uv=pred_uv.astype(np.float32),
